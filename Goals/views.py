...
def goals_dashboard(request):
    ...
    return render(request, 'new/user_goals_landing_page.html', context)
...
def my_goals(request):
    ...
    context = {'user_profile': user_profile, 'user_notifications': user_notifications,
               ...
               'interest_rate': Interest_Rate.objects.get(pk=1),'total_complete':total_complete
...
def express_saving(request):
    ...
    if request.method == 'POST':
        ...
        if user_wallet.available_for_withdraw >= amount:
            ...
            express_saving = ExpressSaving.objects.create(user=request.user, amount=amount, is_withdraw='No')
            express_saving.save()
            user_wallet.available_for_withdraw -= float(amount)
            user_wallet.save()
            print('Amount added to express saving:', amount, express_saving.amount)
...
def add_funds_to_group_goal(request):
    if request.method == 'POST':
        ...
        if user_wallet.available_for_withdraw >= amount:
            ...
            GroupGoalMember_contribution.objects.create(group_goal=goal_info, amount=amount, user=request.user).save()
            user_profile = Profile.objects.get(owner=request.user)
            GroupGoalActivites.objects.create(user=request.user, group_goal=goal_info,
                                              content=request.user.first_name + ' ' + request.user.last_name + ' made a deposit of Kshs: ' + str(
                                                  amount) + ' to the group goal.').save()
    ...
    if request.method == 'POST':
        ...
        if UserBankDetails.objects.filter(user_id=user).exists():
            ...
            UserMoneyWithDrawalStatus.objects.create(
                withdrawal_amount=Fv_after_tax,
                status="Pending...User request For Withdraw money from express saving",
                withraw_for="Persoanl Goal of title" + str(goal_info.name),
                user_id=user
...
def withdraw_money_group_goal(request):
    if request.method == 'POST':
        ...
        if UserBankDetails.objects.filter(user_id=user).exists():
            ...
            send_sms(phone_data_allpending.phone, 'Congratulation',
                     'We receive withdraw request for goal title ' + str(
                         goal_info.goal_name) + ' of amount Kshs: ' + str(
                         goal_info.achieved_amount) + ' We are processing the transaction, and you will receive a confirmation once it is successfully completed. ')

            search_str = UserFcmTokens.objects.filter(user=user).order_by('-token')[:1]
            send_notif(search_str, None, True, True, "Congratulation",
                       'We receive withdraw request for goal title ' + str(
                           goal_info.goal_name) + ' of amount ' + str(
                           goal_info.achieved_amount) + ' We are processing the transaction, and you will receive a confirmation once it is successfully completed. ',
                       None, False,
                       user)
            ...
            for item in x:
                ...
                send_sms(phone_data_allpending.phone, 'Congratulation',
                         str(goal_info.creator.first_name) + ' You have submitted a withdrawal request for the goal  ' + str(
                             goal_info.goal_name) + ' Your available balance on My Account has been updated.')

                search_str = UserFcmTokens.objects.filter(user=item.user).order_by('-token')[:1]
                send_notif(search_str, None, True, True, "Congratulation",
                           str(goal_info.creator.username) + ' You have submitted a withdrawal request for the goal ' + str(
                               goal_info.goal_name) + ' Your available balance on My Account has been updated. ',
                           None, False,
                           user)
