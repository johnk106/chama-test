...
class DownloadService:
    ...
    def download_individual_investments(request, chama_id):
        # 1) Fetch Chama and individual investments
        chama = Chama.objects.get(pk=chama_id)
        member_id = request.GET.get('member_id', None)
        start_date = request.GET.get('start_date', None)
        end_date = request.GET.get('end_date', None)
        
        # Base queryset for individual investments
        investments = Investment.objects.filter(chama=chama, forGroup=False).select_related('owner')
        
        # Apply filters
        if member_id:
            investments = investments.filter(owner_id=member_id)
        if start_date:
            investments = investments.filter(date__date__gte=start_date)
        if end_date:
            investments = investments.filter(date__date__lte=end_date)
        ...
        # 2) Create PDF response
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename="individual_investments_report.pdf"'
        ...
        # 4) Define table data
        data = [['Member', 'Investment Name', 'Amount', 'Date']]
        for investment in investments:
            data.append([
                investment.owner.name if investment.owner else 'Unknown',
                investment.name,
                f'ksh {investment.amount}',
                investment.date.strftime('%Y-%m-%d') if investment.date else ''
            ])
    ...
    def download_my_investments(request, chama_id):
        ...
        chama = Chama.objects.get(pk=chama_id)
        user = request.user
        member = ChamaMember.objects.get(user=user, group=chama)
        
        start_date = request.GET.get('start_date', None)
        end_date = request.GET.get('end_date', None)
        
        # Base queryset for my investments
        investments = Investment.objects.filter(chama=chama, owner=member)
        ...
        # 2) Create PDF response
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename="my_investments_report.pdf"'
